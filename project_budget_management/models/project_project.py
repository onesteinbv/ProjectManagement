...
class ProjectProject(models.Model):
    ...
    def _get_cost_items_from_vendor_bills(self):
        ...
        query_string, query_param = query.select('price_total', 'parent_state', 'account_move_line.currency_id',
        ...
        if invoices_move_line_read:
            ...
            for moves_read in invoices_move_line_read:
                price_total = self.currency_id.round(
                    moves_read['price_total'] * conversion_rates[moves_read['currency_id']])
                ...
                if moves_read['parent_state'] == 'draft':
                    if moves_read['move_type'] == 'in_invoice':
                        amount_to_invoice += price_total * analytic_contribution
                    else:  # moves_read['move_type'] == 'out_refund'
                        amount_to_invoice -= price_total * analytic_contribution
                else:  # moves_read['parent_state'] == 'posted'
                    if moves_read['move_type'] == 'in_invoice':
                        amount_invoiced += price_total * analytic_contribution
                    else:  # moves_read['move_type'] == 'out_refund'
                        amount_invoiced -= price_total * analytic_contribution
